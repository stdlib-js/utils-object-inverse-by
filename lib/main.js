...
function invertBy( obj, opts, transform ) {
	...
	var i;
	if ( !isObjectLike( obj ) ) {
		throw new TypeError( format( '1WJOm', obj ) );
	}
	allowDupes = true;
	if ( arguments.length === 2 ) {
		...
		if ( !isObject( opts ) ) {
			throw new TypeError( format( '1WJ2V,FD', opts ) );
		}
		if ( hasOwnProp( opts, 'duplicates' ) ) {
			allowDupes = opts.duplicates;
			if ( !isBoolean( allowDupes ) ) {
				throw new TypeError( format( '1WJ2o,GE', 'duplicates', allowDupes ) );
			}
		}
		cb = transform;
	}
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '1WJ3q,JV', cb ) );
	}
	keys = objectKeys( obj );
	len = keys.length;
	out = {};
